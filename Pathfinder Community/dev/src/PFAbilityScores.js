...
var columnMods = ['-base', '-racial', '-asi', '-enhance', '-inherent', '-misc', '-damage', '-penalty', '-drain', '-mod', '-cond', '-modded'],
  ...
  events = {
    ...
    abilityEventsPlayer:
      'change:REPLACE-base change:REPLACE-racial change:REPLACE-asi change:REPLACE-enhance change:REPLACE-inherent change:REPLACE-misc change:REPLACE-temp change:REPLACE-damage change:REPLACE-penalty change:REPLACE-drain',
  };
...
export function getAttributes(ability) {
  var fields = _.map(columnMods, function (col) {
    return ability + col;
  });
  fields.push(ability);
  fields = fields.concat(
    _.map(columnBuffMods, function (col) {
...
function getAbilityModUpdates(abilityModName, newval, v, setter) {
  setter = setter || {};
  return Object.keys(PFConst.abilityScoreManualDropdowns)
    .filter(function (a) {
    ...
    if (isNaN(base)) {
      ...
      if (ability === 'DEX' && (helpless || paralyzed)) {
        ...
        newVal =
          ...
          (parseInt(values[ability + '-inherent'], 10) || 0) +
          (parseInt(values[ability + '-misc'], 10) || 0) +
          (parseInt(values[ability + '-drain'], 10) || 0) +
          (parseInt(values['buff_' + ability + '-total'], 10) || 0);
...
export function applyConditions(callback, silently, eventInfo) {
  getAttrs(
    ['STR-cond', 'DEX-cond', 'condition-Helpless', 'condition-Paralyzed', 'condition-Exhausted', 'condition-Fatigued', 'condition-Entangled', 'condition-Grappled'],
    function (v) {
      var setter = {},
        silentSetter = {},
        params = {},
...
function registerEventHandlers() {
  ...
  _.each(abilities, function (ability) {
    on(
      events.abilityEventsAuto.replace(/REPLACE/g, ability),
    ...
    on(
      events.abilityEventsPlayer.replace(/REPLACE/g, ability),
  ...
  abilitymods.forEach(function (attr) {
    on(
      'change:' + attr,
      TAS.callback(function eventAbilityModUpdate(eventInfo) {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
        if (eventInfo.sourceType === 'sheetworker' || eventInfo.sourceType === 'api') {
