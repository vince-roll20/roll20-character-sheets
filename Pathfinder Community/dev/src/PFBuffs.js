...
export var //values in the bonus dropdown
  buffColumns = [
    'ac',
    'armor',
    'attack',
  ...
  buffToTot = {
    ...
    ranged2: 'ranged2',
    cmb2: 'cmb2',
    dmg_melee2: 'dmg_melee2',
    dmg_ranged2: 'dmg_ranged2',
    kineticblast: 'kineticblast',
    dmg_kineticblast: 'dmg_kineticblast',
    dmg_power_attack: 'dmg_power_attack',
    customa1: 'customa1',
    customa2: 'customa2',
    customa3: 'customa3',
    customa4: 'customa4',
...
var armorcols = ['armor', 'shield', 'natural'],
  ...
  charBonusFields = _.chain(otherCharBonuses)
    .values()
    .map(function (v) {
      return _.values(v);
    })
...
function getBuffNotes(id, v, setter) {
  ...
  try {
    ...
    if (notefield) {
      ...
      if (!tempstr) {
        if (addNote) {
          output[notefield] = notestr;
        }
      } else if (notereg.test(tempstr)) {
        if (!addNote) {
          output[notefield] = tempstr.replace(notereg, '');
        }
      } else if (addNote) {
        output[notefield] = tempstr + notestr;
      }
...
function addNoteAsync(id, eventInfo) {
  ...
  const rowId = id || SWUtils.getRowId(eventInfo.sourceAttribute);
  idStr = SWUtils.getRepeatingIDStr(rowId);
  prefix = 'repeating_buff2_' + idStr;

  fields = buffNoteFields.concat([prefix + 'add_note_to_roll', prefix + 'enable_toggle']);
  //TAS.debug("the fields are ",fields);
  getAttrs(fields, function (v) {
    var setter = getBuffNotes(rowId, v);
    if (_.size(setter)) {
...
function assembleRows(ids, v, col) {
  ...
  // Ensure ids is an array to prevent crashes before reduce
  const safeIds = ids || [];
...
function updateBuffTotal(col, rows, v, setter) {
  ...
  try {
    ...
    TAS.debug('######################', 'PFBuffs setting ', output);
    return output;
  }
}
  ...
  getSectionIDs('repeating_buff2', function (ids) {
    ...
    if (ids && _.size(ids)) {
      try {
        ...
        totals = columnsToUpdate.map(function (b) {
          return buffToTot[b];
        });
        if (col === 'ac') {
          totals.push('dodge');
        ...
        totfields = totals
          .map(function (t) {
            return 'buff_' + t + '-total';
          })
          .concat(
            totals.map(function (t) {
              return 'buff_' + t + '_exists';
            }),
          );
        if (isAbility) {
      ...
      getAttrs(fields, function (v) {
        ...
        try {
          ...
          const rowIds = ids.filter(function (id) {
            return parseInt(v['repeating_buff2_' + id + '_enable_toggle'], 10) || 0;
          });
          //TAS.debug("PFBuffsasync there are " +_.size(rowIds)+" rows" );
          if (_.size(rowIds)) {
            rows = assembleRows(rowIds, v, col);
  ...
  getSectionIDs('repeating_buff2', function (ids) {
    ...
    getAttrs(fields, function (v) {
      var rows = [],
        params = {},
        setter = {};
      let values = _.clone(v);
      try {
        const rowids = ids.filter(function (id) {
          return parseInt(v['repeating_buff2_' + id + '_enable_toggle'], 10) || 0;
        });
        if (!rowids || _.size(rowids) === 0) {
          clearBuffTotals(done, silently);
          return;
        }
        ...
        rowids.forEach(function (id) {
          const notes = getBuffNotes(id, values, setter);
          _.extend(setter, notes);
          _.extend(values, notes);
        });
        rows = assembleRows(rowids, values);
        _.each(buffColumns, function (col) {
          updateBuffTotal(col, rows, values, setter);
        });
      } catch (errou) {
        TAS.error('PFBuffs.updateAllBuffTotalsAsync2 errou', errou);
      } finally {
        if (_.size(setter)) {
  ...
  getAttrs(buffTotFields, function (v) {
    var setter = {},
      getExists = function (pre, post) {
        var val, exists;
        const postText = post || '';
        val = parseInt(v[pre + '-total' + postText], 10) || 0;
        exists = parseInt(v[pre + postText + '_exists'], 10) || 0;
        if (val !== 0 && !exists) {
          return 1;
        }
        if (val === 0 && exists) {
    ...
    recalculateBuffRow = function (callback, id, v) {
      ...
      try {
        buffsPerRow.forEach(function (b) {
          if (parseInt(v['repeating_buff2_' + id + '_enable_toggle'], 10) && parseInt(v['repeating_buff2_' + id + '_' + b + '-show'], 10)) {
...
/** Creates common buffs from dropdown
 *
...
function getCommonBuffEntries(name, v, onByDefault) {
  ...
  switch (name) {
    ...
    case 'aid':
      ...
      const random1d8 = Math.floor(Math.random() * 8) + 1;
      setter[prefix + 'b3_macro-text'] = `${random1d8}[1d8] + min(10, @{level})`;
      if (calc) {
        tempint = 4 + level;
      } else {
        tempint = 4;
    ...
    case 'slow':
      ...
      setter[prefix + 'b4-show'] = 1;
      setter[prefix + 'b4_bonus'] = 'speed';
      setter[prefix + 'b4_macro-text'] = '-ceil(ceil(@{speed-base}/2)/5)*5';
      setter[prefix + 'b4_val'] = -15;
      setter[prefix + 'description-show'] = '1';
    ...
    case 'elementaloverflow':
      ...
      setter[prefix + 'notes'] =
        '**:' +
        SWUtils.getTranslated('buff-elemental-overflow') +
        '** [[({1d1,{1d0,(@{kineticist_level-mod}-2)d1}kh1}kl1)*5*@{kineticistburn}]]**%** ' +
        SWUtils.getTranslated('buff-elemental-overflow-note');
      break;
    case 'powerattack':
...
function registerEventHandlers() {
  var custombuffs = ['customa1', 'customa2', 'customa3', 'customa4', 'customa5', 'customa6', 'customa7', 'customa8', 'customa9', 'customa10', 'customa11', 'customa12'];

  on(
    'change:add_common_buff',
    TAS.callback(function eventAddCommonBuff(eventInfo) {
  ...
  _.each(otherCharBonuses, function (charFieldMap, buff) {
    _.each(charFieldMap, function (field, bonustype) {
      on(
        'change:' + field,
        TAS.callback(function eventCharFieldUpdatesBuff(eventInfo) {
          if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
            TAS.debug('caught ' + eventInfo.sourceAttribute + ', event: ' + eventInfo.sourceType);
  ...
  _.each(charHelperFields, function (charFieldArray, buff) {
    _.each(charFieldArray, function (field) {
      on(
        'change:' + field,
        TAS.callback(function eventCharBuffHelperField(eventInfo) {
          if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
            TAS.debug('caught ' + eventInfo.sourceAttribute + ', event: ' + eventInfo.sourceType);
            updateBuffTotalAsync(buff);
          }
  ...
  buffsPerRow.forEach(function (b) {
    ...
    on(
      prefix + '_bonustype',
      TAS.callback(function PFBuffs_updateBuffbonustype(eventInfo) {
        if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
          getAttrs(['repeating_buff2_' + b + '_val', 'repeating_buff2_' + b + '-show', 'repeating_buff2_' + b + '_bonus', 'repeating_buff2_enable_toggle'], function (v) {
            ...
            if (
              parseInt(v['repeating_buff2_' + b + '-show'], 10) &&
              parseInt(v['repeating_buff2_enable_toggle'], 10) &&
              parseInt(v['repeating_buff2_' + b + '_val'], 10) &&
    ...
    on(
      prefix + '-show ',
      TAS.callback(function PFBuffs_updateBuffRowShowBuff(eventInfo) {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
        if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
          getAttrs(['repeating_buff2_' + b + '_val', 'repeating_buff2_' + b + '_bonus', 'repeating_buff2_enable_toggle'], function (v) {
            TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType, v);
            if (parseInt(v['repeating_buff2_enable_toggle'], 10) && parseInt(v['repeating_buff2_' + b + '_val'], 10) && v['repeating_buff2_' + b + '_bonus']) {
              updateBuffTotalAsync(v['repeating_buff2_' + b + '_bonus']);
            }
          });
        }
    ...
    on(
      prefix + '_val',
      TAS.callback(function updateBuffValue(eventInfo) {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
    ...
    on(
      prefix + '_bonus',
      TAS.callback(function PFBuffs_updateBuffbonus(eventInfo) {
        if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:repeating_buff2:add_note_to_roll',
    TAS.callback(function PFBuffs_addnote(eventInfo) {
  ...
  on(
    'change:repeating_buff2:enable_toggle',
    TAS.callback(function PFBuffs_enabletoggle(eventInfo) {
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        getAttrs(
          [
            'repeating_buff2_b1_bonus',
            'repeating_buff2_b2_bonus',
            'repeating_buff2_b3_bonus',
            'repeating_buff2_b4_bonus',
            'repeating_buff2_b5_bonus',
            'repeating_buff2_b6_bonus',
            'repeating_buff2_b1-show',
            'repeating_buff2_b2-show',
            'repeating_buff2_b3-show',
            'repeating_buff2_b4-show',
            'repeating_buff2_b5-show',
            'repeating_buff2_b6-show',
            'repeating_buff2_enable_toggle',
            'repeating_buff2_tabcat2',
          ],
          function (v) {
            var setter = {};
            TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType, v);
  ...
  on(
    'change:repeating_buff2:bufftype',
    TAS.callback(function eventBuff2Type(eventInfo) {
      var setter = {};
      if (eventInfo.sourceType === 'player') {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
  ...
  on(
    'remove:repeating_buff2',
    TAS.callback(function PFBuffs_removeBuffRow(eventInfo) {
  ...
  _.each(events.buffTotalNonAbilityEvents, function (functions, col) {
    ...
    _.each(functions, function (methodToCall) {
      on(
        eventToWatch,
        TAS.callback(function event_updateBuffNonAbilityEvents(eventInfo) {
          TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
          if (eventInfo.sourceType === 'sheetworker' || eventInfo.sourceType === 'api') {
  ...
  _.each(events.buffTotalAbilityEvents, function (functions, col) {
    ...
    _.each(functions, function (methodToCall) {
      on(
        eventToWatch,
        TAS.callback(function event_updateBuffAbilityEvents(eventInfo) {
          TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
          if (eventInfo.sourceType === 'sheetworker' || eventInfo.sourceType === 'api') {
            methodToCall(col, eventInfo);
          }
  ...
  _.each(events.buffTotalEventsNoParam, function (functions, col) {
    ...
    _.each(functions, function (methodToCall) {
      on(
        eventToWatch,
        TAS.callback(function eventBuffTotalNoParam(eventInfo) {
          TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
          if (eventInfo.sourceType === 'sheetworker' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:merge_buffs_now',
    TAS.callback(function eventMergeBuffs(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player') {
  ...
  custombuffs.forEach(function (buff) {
    on(
      'change:buff_' + buff + '-total',
      TAS.callback(function customBuffTotal(eventInfo) {
        if (eventInfo.sourceType === 'sheetworker' || eventInfo.sourceType === 'api') {
          TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
