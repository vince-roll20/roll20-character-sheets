...
export var defenseDropdowns = {
  ...
  events = {
    defenseEventsAuto:
      'change:bab change:ac-penalty change:cmd-penalty change:size change:ac-shield change:ac-armor change:ac-ability-mod change:ff-dex change:cmd-dex change:ff-cmd-dex change:cmd-str change:max-dex change:ac-misc-mod change:cmd-misc-mod',
...
export function updateDefenses(callback, silently, eventInfo) {
  ...
  getAttrs(
    [
      ...
      'buff_armor-total',
      'buff_shield-total',
      'buff_flat-footed-total',
      'buff_natural-total',
      'buff_dodge-total',
      'buff_deflection-total',
      'buffsumac',
      'buffsumtouch',
      'buffsumff',
      'buffsumcmd',
      ...
      if (dropdownMixed) {
        getAttrs(['unlock_def_ability', 'uncanny_dodge'], function (v) {
          if (parseInt(v['unlock_def_ability'], 10) || dropdownLower === 'ac-ability' || dropdownLower === 'cmd-ability1') {
            PFUtilsAsync.setDropdownValue(
              dropdownMixed,
              defenseDropdowns[dropdownMixed],
              function (newv, oldv) {
    ...
    function (v) {
      ...
      try {
        ...
        if (!eventInfo || !/grappled|invisible/i.test(eventInfo.sourceAttribute)) {
          ...
          subTotPenalty =
            -1 *
            ((parseInt(v['condition-Blinded'], 10) || 0) +
              (parseInt(v['condition-Pinned'], 10) || 0) +
              (parseInt(v['condition-Cowering'], 10) || 0) +
              (parseInt(v['condition-Stunned'], 10) || 0));
...
export function migrate(callback, oldversion) {
  ...
  if (oldversion > 0 && oldversion < 0.5) {
    PFMigrate.migrateMaxDexAndACP();
  }
  if (oldversion > 0 && oldversion < 1.2) {
...
function registerEventHandlers() {
  _.each(defenseDropdowns, function (write, read) {
    on(
      'change:' + read,
      TAS.callback(function eventsetDefenseDropdownMod(eventInfo) {
  ...
  on(
    'change:uncanny_dodge',
    TAS.callback(function eventUncannyDodgeUpdate(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
  ...
  on(
    'change:hd_not_bab',
    TAS.callback(function eventCMDSwitchHDandBAB(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    events.defenseEventsAuto,
    TAS.callback(function eventUpdateDefensesAuto(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
  ...
  on(
    events.defenseEventsPlayer,
    TAS.callback(function eventUpdateDefensesPlayer(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        updateDefenses(null, null, eventInfo);
      }
  ...
  on(
    events.defenseEventsEither,
    TAS.callback(function eventUpdateDefensesEither(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      updateDefenses(null, null, eventInfo);
    }),
  );
  on(
    'change:max-dex-source change:current-load',
