...
export var optionToggles = [
    'toggle_spell_school_notes',
    ...
    'toggle_spell_spellnotes_notes',
    'toggle_spell_spell_fail_check',
    'toggle_spell_damage_notes',
    'toggle_spell_spellPen_check',
    'toggle_spell_dc_notes',
  ],
  optionTemplates = {
...
export function updateSpellOption(eventInfo, fieldUpdated) {
  ...
  getAttrs(
    [fieldName, 'repeating_spells_spell_options', 'repeating_spells_spell_lvlstr', toggleField, 'repeating_spells_sr', 'repeating_spells_SP-mod', 'repeating_spells_savedc'],
    function (v) {
...
export function getOptionText(id, eventInfo, toggleValues, rowValues) {
  ...
  if (toggleValues.showsaving_throw && toggleValues.showdc) {
    //checks to see if DC should be included
  ...
  if ((toggleValues.showconcentration || toggleValues.showconcentration_check) && toggleValues.showdc) {
    //checks to see if DC should be included
    if (defMod > 0) {
...
export function resetOption(id, eventInfo) {
  ...
  getAttrs(allFields, function (v) {
    var toggleValues = _.chain(optionToggles)
...
export function resetOptions(callback, eventInfo) {
  ...
  getAttrs(optionTogglesPlusOptionAttrs, function (tv) {
    ...
    try {
      ...
      toggleValues = _.chain(optionToggles)
        .reduce(function (memo, attr) {
...
function registerEventHandlers() {
  ...
  _.each(optionToggles, function (toggleField) {
    on(
      'change:' + toggleField,
      TAS.callback(function toggleSpellOptionField(eventInfo) {
        if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
          TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
